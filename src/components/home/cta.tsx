import Link from "next/link";

export function Cta() {
  return (
    <section className="relative px-5 sm:px-8 lg:px-20 py-10 lg:py-20 text-center">
      <div className="mx-auto flex max-w-[640px] flex-col items-center gap-6">
        <h2 className="font-serif text-heading-lg font-normal leading-[1.2] text-bg-warm">
          Looking for a trusted partner.
        </h2>
        <p className="max-w-[480px] text-base leading-[1.7] text-[#cddaf0]">
          E Malan &amp; Associates strives to make you feel comfortable and
          confident in your accounts. Schedule a call today to find out
          more.
        </p>
        <Link
          href="/contact"
          className="mt-2 rounded-[19px] bg-bg-warm px-8 py-4 text-[13px] sm:text-[14px] lg:text-[15px] font-semibold text-navy-primary transition-opacity hover:opacity-90"
        >
          Schedule a Call
        </Link>
      </div>
    </section>
  );
}
